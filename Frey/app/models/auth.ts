...
export async function approveResetPassword(
  ...
  if (
    payload.resetToken === "" ||
    regData[0].auth_reset_token !== payload.resetToken
  ) {
...
export async function changePassword(
  ...
  await verifyHash(
    makeClientHash(payload.password, regData[0].auth_salt),
    regData[0].auth_hash,
  );
...
export async function requestResetPassword(
  ...
  await db.updateTable("person").set({ auth_reset_token: token }).execute();

  if (payload.sendEmail) {
    await sendPasswordRecovery(token, payload.email);
    return { resetToken: "" };
  }
