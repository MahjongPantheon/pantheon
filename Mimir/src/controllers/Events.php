...
class EventsController extends Controller
{
    ...
    /**
     * Timer state for particular session
     *
     ...
     * @throws InvalidParametersException
     */
    public function getTimerStateForSession(string $sessionHash)
    {
        $this->_log->info('Getting timer for session id#' . $sessionHash);

        $session = SessionPrimitive::findByRepresentationalHash($this->_ds, [$sessionHash]);
        if (empty($session)) {
            throw new TwirpError(ErrorCode::NotFound, 'Session id#' . $sessionHash . ' not found in DB');
        }

        $event = $session[0]->getEvent();
        if (empty($event)) {
            throw new TwirpError(ErrorCode::NotFound, 'Event for session #' . $sessionHash . ' not found in DB');
        }

        if (!$event->getUseTimer()) {
            $this->_log->info('Timer is not used for event id#' . $event->getId());
            return [];
        }
        ...
        if (!$event->getIsFinished()) {
            ...
            } else if ($event->getLastTimer() + $event->getGameDuration() * 60 + $session[0]->getExtraTime() > time()) {
                // game in progress
                $response = [
                    'started' => true,
                    'finished' => false,
                    'time_remaining' => $event->getLastTimer() + $event->getGameDuration() * 60 + $session[0]->getExtraTime() - time()
                ];
            }
        ...
        $this->_log->info('Successfully got timer data for session id#' . $sessionHash);
