...
export class Migration20260521213109 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`comment on column "event_prescript"."script" is 'predefined event seating script';`);
    this.addSql(`alter table "event_prescript" add constraint "event_prescript_event_id_unique" unique ("event_id");`);
    ...
    this.addSql(`comment on column "session"."orig_link" is 'original tenhou game link, for access to replay';`);
    ...
    this.addSql(`create table "hand" ("id" serial primary key, "round_id" int not null, "han" int null, "fu" int null, "dora" int null, "uradora" int null, "kandora" int null, "kanuradora" int null, "yaku" jsonb null, "tempai" jsonb null, "nagashi" jsonb null, "winner_id" int null, "loser_id" int null, "pao_player_id" int null, "open_hand" boolean null);`);
    ...
    this.addSql(`create table "session_results" ("id" serial primary key, "event_id" int not null, "session_id" int not null, "player_id" int not null, "place" int not null, "score" int not null, "rating_delta" int not null, "chips" int null);`);
    this.addSql(`comment on column "session_results"."score" is 'how many points player has at the end, before any uma/oka calc';`);
    this.addSql(`comment on column "session_results"."rating_delta" is 'resulting score after uma/oka and starting points subtraction';`);

    this.addSql(`alter table "achievements" add constraint "achievements_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);

    this.addSql(`alter table "event_prescript" add constraint "event_prescript_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);

    this.addSql(`alter table "event_registered_players" add constraint "event_registered_players_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);

    this.addSql(`alter table "player_history" add constraint "player_history_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);

    this.addSql(`alter table "player_stats" add constraint "player_stats_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);

    this.addSql(`alter table "session" add constraint "session_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);

    this.addSql(`alter table "round" add constraint "round_session_id_foreign" foreign key ("session_id") references "session" ("id") on update cascade;`);
    this.addSql(`alter table "round" add constraint "round_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);

    this.addSql(`alter table "hand" add constraint "hand_round_id_foreign" foreign key ("round_id") references "round" ("id") on update cascade;`);

    this.addSql(`alter table "penalty" add constraint "penalty_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);
    this.addSql(`alter table "penalty" add constraint "penalty_session_id_foreign" foreign key ("session_id") references "session" ("id") on update cascade on delete set null;`);

    this.addSql(`alter table "session_players" add constraint "session_players_session_id_foreign" foreign key ("session_id") references "session" ("id") on update cascade;`);

    this.addSql(`alter table "session_results" add constraint "session_results_event_id_foreign" foreign key ("event_id") references "event" ("id") on update cascade;`);
    this.addSql(`alter table "session_results" add constraint "session_results_session_id_foreign" foreign key ("session_id") references "session" ("id") on update cascade;`);
  }
