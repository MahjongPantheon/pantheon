...
export function getPlayerWithPao(state: IAppState) {
  switch (state.currentOutcome?.selectedOutcome) {
    case RoundOutcome.ROUND_OUTCOME_TSUMO:
      return { [state.currentOutcome.winner!]: state.currentOutcome.paoPlayerId };
    case RoundOutcome.ROUND_OUTCOME_RON:
      return Object.fromEntries(
        Object.entries(state.currentOutcome.wins).map(([playerId, data]) => {
          return [playerId, data.paoPlayerId];
        })
      );
    default:
      return {};
