...
/**
 * @internal HuginAbstractClient provides abstraction for JsonClient and Client (default).
 ...
 */
abstract class HuginAbstractClient
{
    /**
     * @var server
     */
     ...
     * {@inheritdoc}
     */
    public function GetLastDay(array $ctx, \Common\GetLastDayPayload $in): \Common\GetLastDayResponse
    {
        $ctx = Context::withPackageName($ctx, 'common');
        $ctx = Context::withServiceName($ctx, 'Hugin');
        $ctx = Context::withMethodName($ctx, 'GetLastDay');

        $out = new \Common\GetLastDayResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/common.Hugin/GetLastDay';
        } else {
            $url = $url.'/'.$this->prefix.'/common.Hugin/GetLastDay';
        }
     ...
     * {@inheritdoc}
     */
    public function GetLastMonth(array $ctx, \Common\GetLastMonthPayload $in): \Common\GetLastMonthResponse
    {
        $ctx = Context::withPackageName($ctx, 'common');
        $ctx = Context::withServiceName($ctx, 'Hugin');
        $ctx = Context::withMethodName($ctx, 'GetLastMonth');

        $out = new \Common\GetLastMonthResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/common.Hugin/GetLastMonth';
        } else {
            $url = $url.'/'.$this->prefix.'/common.Hugin/GetLastMonth';
        }
     ...
     * {@inheritdoc}
     */
    public function GetLastYear(array $ctx, \Common\GetLastYearPayload $in): \Common\GetLastYearResponse
    {
        $ctx = Context::withPackageName($ctx, 'common');
        $ctx = Context::withServiceName($ctx, 'Hugin');
        $ctx = Context::withMethodName($ctx, 'GetLastYear');

        $out = new \Common\GetLastYearResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/common.Hugin/GetLastYear';
        } else {
            $url = $url.'/'.$this->prefix.'/common.Hugin/GetLastYear';
        }
