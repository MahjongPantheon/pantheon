 ...
 */
final class HuginServer implements RequestHandlerInterface
{
    /**
     ...
     *
     * Use HuginServer::getPathPrefix instead.
     */
    public const PATH_PREFIX = '/twirp/common.Hugin/';
    ...
    {
        return $this->prefix.'/common.Hugin/';
    }
    ...
    public function handle(ServerRequestInterface $req): ResponseInterface
    {
        ...
        $ctx = Context::withServiceName($ctx, 'Hugin');
        ...
        switch ($method) {
            case 'GetLastDay':
                return $this->handleGetLastDay($ctx, $req);
            case 'GetLastMonth':
                return $this->handleGetLastMonth($ctx, $req);
            case 'GetLastYear':
                return $this->handleGetLastYear($ctx, $req);
    ...
    private function handleGetLastDay(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleGetLastDayJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleGetLastDayProtobuf($ctx, $req);
                break;
    ...
    private function handleGetLastDayJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetLastDay');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Common\GetLastDayPayload();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->GetLastDay($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling GetLastDay. null responses are not supported'));
            }
    ...
    private function handleGetLastDayProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetLastDay');
    ...
    private function handleGetLastMonth(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleGetLastMonthJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleGetLastMonthProtobuf($ctx, $req);
                break;
    ...
    private function handleGetLastMonthJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetLastMonth');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Common\GetLastMonthPayload();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->GetLastMonth($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling GetLastMonth. null responses are not supported'));
            }
    ...
    private function handleGetLastMonthProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetLastMonth');
    ...
    private function handleGetLastYear(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $header = $req->getHeaderLine('Content-Type');
        $i = strpos($header, ';');
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handleGetLastYearJson($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handleGetLastYearProtobuf($ctx, $req);
                break;
    ...
    private function handleGetLastYearJson(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetLastYear');

        try {
            $ctx = $this->hook->requestRouted($ctx);

            $in = new \Common\GetLastYearPayload();
            $in->mergeFromJsonString((string)$req->getBody(), true);

            $out = $this->svc->GetLastYear($ctx, $in);

            if ($out === null) {
                return $this->writeError($ctx, TwirpError::newError(ErrorCode::Internal, 'received a null response while calling GetLastYear. null responses are not supported'));
            }
    ...
    private function handleGetLastYearProtobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, 'GetLastYear');
