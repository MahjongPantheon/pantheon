...
export class UserItemComponent extends I18nComponent {
  ...
  get showNagashiButton() {
    return -1 !== ['nagashi']
      .indexOf(this.state.getOutcome()) && !this.paoSelectionMode && this.nagashiSelectionMode;
  }

  get winPressed() {
    return -1 !== this.state.getWinningUsers()
      .indexOf(this.userData);
  }

  get losePressed() {
    return -1 !== this.state.getLosingUsers()
      .indexOf(this.userData);
  }

  get paoPressed() {
    return -1 !== this.state.getPaoUsers()
      .indexOf(this.userData);
  }

  get riichiPressed() {
    return -1 !== this.state.getRiichiUsers()
      .indexOf(this.userData);
  }

  get deadPressed() {
    return -1 !== this.state.getDeadhandUsers()
      .indexOf(this.userData);
  }

  get nagashiPressed() {
    return -1 !== this.state.getNagashiUsers()
      .indexOf(this.userData);
  }
