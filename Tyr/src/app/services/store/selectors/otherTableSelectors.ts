...
type WinState = Array<{ winner: string, loser: string, han: number, fu: number, dora: number, yakuList: string }>;
function _getWins(info: RRoundPaymentsInfo, players: Player[], i18n: I18nService): WinState {
  switch (info.outcome) {
    case 'ron':
    case 'tsumo':
      return [{
