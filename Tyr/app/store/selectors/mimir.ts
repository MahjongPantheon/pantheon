...
function _hasYaku(state: IAppState, id: YakuId) {
  ...
  switch (outcome?.selectedOutcome) {
    case RoundOutcome.ROUND_OUTCOME_TSUMO:
      return { [outcome.winner!]: -1 !== unpack(outcome.yaku).indexOf(id) };
    case RoundOutcome.ROUND_OUTCOME_RON:
      return Object.keys(outcome.wins).reduce(
        (acc, win) => {
          acc[outcome.wins[win].winner!] = -1 !== unpack(outcome.wins[win].yaku).indexOf(id);
          return acc;
        },
