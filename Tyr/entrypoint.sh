...
/usr/local/bin/node_exporter --web.listen-address=:9110 2>&1 &
NX_PID=$!

trap "TRAPPED_SIGNAL=true; kill -15 $NGINX_PID; kill -15 $NX_PID" SIGTERM  SIGINT
...
do
    ...
    if [ "$TRAPPED_SIGNAL" = "false" ]; then
        if [ $NGINX_STATUS -ne 0 ] || [ $NX_STATUS -ne 0 ]; then
